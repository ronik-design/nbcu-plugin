...
if( $authProcessor->urlAuthPageEnabled($_SERVER['REQUEST_URI']) == 'VALID' ){
...
function ronikdesigns_redirect_registered_auth() {
    ...
    // Store user Point of Origin
        // We check if no special wp page is current.
        if( $authProcessor->urlCheckNoWpPage($_SERVER['REQUEST_URI']) ){
            // We check if no authentication page is current.
            if( $authProcessor->urlCheckNoAuthPage($_SERVER['REQUEST_URI']) ){
                // Run the userTrackerActions function that saves users point of origin.
    ...
    // Get ADMIN preference.
        ...
        if($f_admin_auth_select['mfa'] && !$f_admin_auth_select['2fa'] ){
            ...
            $authProcessor->ronikdesigns_redirect_non_registered_mfa();
            return;
            ...
            $authProcessor->ronikdesigns_redirect_registered_2fa();
            return;
        }
    // Get user preference.
